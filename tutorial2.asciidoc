...
In `lablqml` starting from version `0.3` external JSON file for specifying API is deprecated, PPX extension is required. That's why, `lablqml 0.3` requires OCaml 4.02 or newer.
...
will probably require small adjustments for `lablqml` >= `0.6`.
...
There are some issues https://bugreports.qt-project.org/browse/QTBUG-35256[on Qt bugtracker] about using `pkg-config` on MacOS. It seems that they handle `-F` option wrong. The solution is mentioned https://github.com/Kakadu/lablqml/issues/17[here].
