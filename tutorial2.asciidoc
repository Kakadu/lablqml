...
----
Let's extend object implementation by clicks counter:
...
When user hits mouse button the counter is incremented and signal with new counter value  is emitted by OCaml method `emit_$(PROPERTY_NAME)Changed`. If Qt properties doesn't notify GUI about changes old values will be used and no changes will be applied.

How exactly QtQuick will use it we declare in `Root.qml`. The new rectangle with text inside will show how many times user have clicked. In this demo QtQuick https://qt-project.org/doc/qt-5/qtquicklayouts-overview.html[layouts] are used to place rectangles vertically. Anchors are not used to implement it.
...
Right side (after colon character) should be a valid ECMAScript expression which is evaluted into property value at runtime. It can be multiline, call some functions and methods and use many properties inside. When one of properties used inside is changed all expression is reevaluated and `text` property gets new value.
...
The `on` prefix is a syntax convention to declare signal handlers (behind the scene they are slots which were mentioned above). Arguments of the signal (the `message` in our example) are made available in the context by QtQuick engine. I.e. Qt infers its' names from signal's declaration. If signal is declared in OCaml side without labels Qt will not be able to get signal's arguments' names. That's why signals should be declared using labeled arguments.
...
If types are simple QML basic types suits well enough. But if something complex is need (like structures with a number of fields inside) then right approach is to create QObject-derived class and put these field inside it.
At the moment objects support in `lablqt` is a little bit awkward but your applications like https://github.com/Kakadu/QOcamlBrowser_quick/[QOcamlBrowser] (screehshot below) can be created without this feature. If you have any ideas how to implement it in better manner I will be glad to hear it.

image::images/tutorial2/img3.png[options="responsive"]

The demo envolving objects can be found http://github.com/Kakadu/lablqt/tree/gh-pages/demos/0.3/demo4[there]. In `controller.ml` some methods and properties which use universal type called `Variant.t` are decalred.
...
In OCaml side value of universal variant is matched and specific string is printed. Constructor ++\`qobject++ contains value of type ++cppobj++ which is a raw pointer to instance of C\++ ++QObject++ class. That's why OCaml object is constructed from this value in the first clause.
...
To avoid pattern matching we can allow to use in properties and methods specific type which will represent raw C++ pointer to QObject. It's not obvious at the moment how good it will be because current approach is more powerful but more verbose. If you have any ideas about this topic I will be glad to mailto:kakadu.hafanana@gmail.com[read] e-mails from you.
...
OCaml+QtQuick applications can start up both from C++ and OCaml. Examples using makefiles are available in links above and at lablqt's http://github.com/Kakadu/lablqt/tree/master/qml[repository]. Also http://github.com/Kakadu/QOcamlBrowser_quick[QOCamlBrowser] is available, it uses ocamlbuild as build system.
...
My experience says that I *should* say that `lablqt` is mature enough for big industrial applications to get some feedback. I hope that after reading this tutorial somebody will try it and send me some notes about the experience.
