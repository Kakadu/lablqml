...
Properties always have name, type and methods that return its values. They can have method to be set by (usually it is called _setter_), properties are read-only otherwise. When property changes it can send _signal_ with the updated value.
...
OCaml+QtQuick applications can start up both from C++ and OCaml. Examples using makefiles are available in links above and at lablqt's http://github.com/Kakadu/lablqt/tree/master/qml[repository]. Also http://github.com/Kakadu/QOcamlBrowser_quick[QOCamlBrowser] is available, it uses ocamlbuild as the build system.
