...
* xref:no_moc[Using `lablqml` without codegeneration]
...
Demos described below and sources of this tutorial can be found in Github http://github.com/Kakadu/lablqml/tree/gh-pages/demos/0.3[repository].

Changelog
---------

Starting from version `0.5` `lablqt` is known as `lablqml`. The rename was proposed because we don't create binding for Qt
itself, most of the library is strongly connected to lablqml.
...
[NOTE]
In the summer 2017 `lablqml` has started to depend on the OCaml  package `ocaml-migrate-parsetree`. Compilation commands
will probalby require small adjustments for `lablqml` >= `0.6`.
...
Our applications can start either from OCaml and C\+\+. Example above starts up from OCaml. See http:github.com/Kakadu/lablqml/tree/gh-pages/demos/0.2/demo3[demos] from previous tutorial to get examples of startup from C++.

Full code of first demo is available on https://github.com/Kakadu/lablqml/tree/gh-pages/demos/0.3/demo1[github].
...
Sources of this demo can be found on https://github.com/Kakadu/lablqml/tree/gh-pages/demos/0.3/demo2[github]. Final window should be similar to the one on this picture:
...
Full code of this demo is available on https://github.com/Kakadu/lablqml/tree/gh-pages/demos/0.3/demo3[github].
...
At the moment objects support in `lablqml` is a little bit awkward but an application like https://github.com/Kakadu/QOcamlBrowser_quick/[QOcamlBrowser] (screenshot below) can be created without this feature. If you have a better way to implement it, I would be glad to hear it.

image::images/tutorial2/img3.png[options="responsive"]

The demo can be found http://github.com/Kakadu/lablqml/tree/gh-pages/demos/0.3/demo4[there]. In `controller.ml` some methods and properties, which use the universal type `Variant.t`, are declared.
...
OCaml+QtQuick applications can start up both from C++ and OCaml. Examples using makefiles are available in links above and at lablqml's http://github.com/Kakadu/lablqml/tree/master/demos[repository] (https://github.com/Kakadu/lablqml/blob/1d0c51792b113308d53d1ff48209a437b7ce5ab1/qml/tutorial_examples/example3/Makefile[This one] seems to be full enough). Also http://github.com/Kakadu/QOcamlBrowser_quick[QOCamlBrowser] is available, it uses ocamlbuild as the build system (but is has some issues at the moment: we need to generate some code on `./configure` stage).
...
[[no_moc]]
Using `lablqml` without codegeneration
--------------------------------------

The one may not be very happy about printing some C++ code to the source files and compiling them
seperately. This approach may introduce compliations when the generated code is far from ideal
and requires adjustments. However, Qt follows the approach with compile-time codegeneration
and `lablqml` supports it as described above.

Also, QtQuick has convenience class http://doc.qt.io/qt-5/qqmlpropertymap.html[QQmlPropertyMap] which
allows you to dynamically (without compile-time codegeneration) add new properties to the subclasses
of `QQmlPropertyMap`. The support of this class contributed to `lablqml` `0.5` by
https://github.com/Orbifx[Orbital Fox].
...
https://wiki.qt.io/V4[ECMAScript engine] in it and you can compile OCaml code using `js_of_ocaml` and run it as a crossplatform application.
...
`lablqml` was used in some industrial applications. Ask https://github.com/Orbifx[Orbital Fox] for more detailed feedback.
