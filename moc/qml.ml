...
let gen_meth ~classname ~ocaml_methname ?(invokable=false) 
  ...
  let locals = 
    if locals_count = 1
    then ["_ans"]
    else "_ans" :: (List.init ~f:(fun n -> sprintf "qqq%d" n) (locals_count -1) )
  in
...
(*  if List.exists args ~f:(function `List _ -> true |  _ -> false) 
  ...
  let rec generate_wrapper ~tab var dest typ : unit =
    ...
    let () = match typ with
      ...
      | `Int    -> print_cpp "%s = Val_int (%s); " dest var
      | `Bool   -> print_cpp "%s = Val_bool(%s); " dest var
      | `Float  -> raise (Unimplemented "float values are not yet implemented")
      | `String -> 
          print_cpp "%s = caml_copy_string(%s.toLocal8Bit().data() );" dest var
  ...
  let call_closure_str = match n with 
    ...
    | _ -> begin
      ...
      List.iter2i args argnames ~f:(fun i arg name ->
        generate_wrapper ~tab:2 name (sprintf "args[%d]" i) arg
      );
  ...
  let new_cpp_var =
    ...
    let f () = 
      incr last_index;
      sprintf "__x%d" !last_index
    in
    f
  in
  let () = 
    let rec to_cpp_conv dest var typ =
      match typ with
        | `Unit    -> assert false
        | `Int     -> print_cpp "  %s = Int_val(%s);\n" dest var
        | `String  -> print_cpp "  %s = QString(String_val(%s));\n" dest var
        | `Bool    -> print_cpp "  %s = Bool_val(%s);\n" dest var
        ...
        | `Tuple xs ->
            ...
            to_cpp_conv leftV  (sprintf "Field(%s,0)" var) a;
            let rightV = new_cpp_var () in
            to_cpp_conv rightV (sprintf "Field(%s,1)" var) b;
            print_cpp "  %s = qMakePair(%s,%s);\n" dest leftV rightV;
        | `List t ->
            ...
            print_cpp "  %s = %s;\n" temp_var var;
            print_cpp "  while (%s != Val_emptylist) {\n" temp_var;
            print_cpp "    %s = Field(%s,0); /* head */\n" head_var temp_var;
            print_cpp "    %s %s;\n" cpp_arg_type temp_cpp_var;
            to_cpp_conv temp_cpp_var head_var t;
            print_cpp "    %s << %s;\n" dest temp_cpp_var;
            print_cpp "  }\n";
    ...
    to_cpp_conv cpp_ans_var "_ans" res
  in
  print_cpp "}\n";
  flush file_cpp
